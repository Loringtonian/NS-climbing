...
run_deposits() { # campaign
  ...
  E env CAMPAIGN_ID=$1 USDC_MINT=$M ACTION=deposit AMOUNT_USD=20 WALLET_FILE=keys/demo2.json npx ts-node scripts/demo_flow.ts
}
...
# ---- emit REHEARSALS.md ----
PA=$(pda rehearsal-victory); PB=$(pda rehearsal-dissolve); PC=$(pda rehearsal-deadman)
...
echo "| Depositor wallets | demo1 \`$D1\` (\$100/V5), demo2 \`$D2\` (\$20/V1) |"
echo "| Proposed payout wallet | \`$PAYOUT\` (ATA \`$PAYOUT_ATA\`) |"
echo
echo "## A · VICTORY — majority + organizer proposal → release"
echo
echo "Campaign \`rehearsal-victory\` · PDA \`$PA\` · [explorer](https://explorer.solana.com/address/$PA?cluster=devnet)"
echo
echo "Expected: after 2/2 payout votes, anyone can release; exactly 120 USDC"
...
echo
echo "Campaign \`rehearsal-dissolve\` · PDA \`$PB\` · [explorer](https://explorer.solana.com/address/$PB?cluster=devnet)"
echo
echo "Expected: 2/2 dissolve votes flip DISSOLVED (terminal); the permissionless"
echo "crank returns exactly \$100 and \$20 to their depositors. Observed below."
echo
...
echo
echo "Campaign \`rehearsal-deadman\` · PDA \`$PC\` · [explorer](https://explorer.solana.com/address/$PC?cluster=devnet) · initialized with a ~2-minute deadline (deadline is an init parameter; the real campaign uses 90 days)"
echo
