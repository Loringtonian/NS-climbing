...
- [ ] **FIRST REAL DEPOSIT — the open blocker.** Pool is $0 / 0 depositors. The mainnet deposit
      flow has never carried a successful transaction; the last three commits were fixes to it
      (Phantom warning → RPC 403 → signTransaction + skipPreflight). Organizer deposits #1 and
      we watch it land, *then* the QR goes up.
...
- [ ] Hackathon submissions (today): MagicBlock Luma · Superteam Earn · live pitches (`HACKATHON.md`).
...
| **First successful mainnet deposit**      | Lorin   | **BLOCKER** — pool is $0/0; deposit flow unproven with real money |
