...
withdraw; committing together is the point. **Votes are weighted by dollars
deposited, not by wallet.** Money leaves the pool by exactly three collective
paths: (1) the organizer proposes a payout address and depositors backing a
...
PDA and the Circle USDC mint** — the program may be deployed before the campaign
is initialized and smoke-tested.
...
3. **Release is DUAL-GATED and DOLLAR-WEIGHTED — organizer proposes, depositors
   dispose.** There is no goal and no pre-set destination. `propose_payout` is
   organizer-only (`has_one = admin`, and campaign creation is pinned to the
   ORGANIZER key); proposing or RE-proposing bumps a proposal epoch, which
   resets the payout tally (receipts vote per-epoch — stale votes stop
   counting). `release` requires a live proposal AND a strict majority of the
   pooled DOLLARS backing it (`payout_vote_amount × 2 > total_escrowed`, u128
   math). Neither side alone can move funds: only the organizer can propose an
   address, and no set of depositors can route funds to an address the
   organizer didn't propose. New deposits enlarge `total_escrowed` and can
   un-make a standing dollar-majority until the electorate re-crosses it.
...
5. **No other privileges exist.** The organizer's only powers are creating
   ...
   redirect, or freeze deposits, and cannot stop the dissolve or deadline
   refund paths. The organizer may deposit like anyone else, and that deposit
   then carries dollar-weighted vote like any other. There is no yield, no
   transferable token, no fee.
6. **A dollar-majority can dissolve.** Every deposit issues a **Supporter
   ...
   serving simultaneously as proof-of-support and as the ballot (weight = its
   own locked `amount`). Any badge-holder may `vote_dissolve` (revocable via
   `unvote_dissolve` while active). A STRICT dollar-majority
   (`dissolve_amount * 2 > total_escrowed`) flips the campaign to DISSOLVED —
   terminal: deposits, proposals and release become permanently impossible and
   the permissionless `refund` crank opens immediately (no waiting for the
   deadline). There is no individual withdraw, so the pool only grows while a
   campaign is active; the refund crank clears each departing badge's weight
   from whichever tallies it backed as it pays out, but it only runs
   post-dissolution or post-deadline, when no live election exists.

**The one trust assumption, stated precisely.** Because votes are
dollar-weighted and only the organizer can propose a destination, the sole way
funds reach a chosen address is: the organizer proposes it AND a dollar-majority
ratifies. An organizer who deposits MORE than the rest of the pool combined can
self-ratify their own proposal — so what a depositor trusts is that the named,
...
2. **Verify the deployed program matches this repo:**
   ...
   solana program dump 2PAg6iMEzPQnfzVmKdeUDctmmCYwts46Y5GEZBUDA4KJ onchain.so -u <cluster>
...
3. **Check upgrade authority** (can the code change under you?):
   ```bash
   solana program show 2PAg6iMEzPQnfzVmKdeUDctmmCYwts46Y5GEZBUDA4KJ -u <cluster>
   ```
   Disclosed plan: the upgrade authority is HELD BY THE ORGANIZER's key
   (`84PE7wqGnj5bBJkcLzB3LviriK5XgF5fUU3VmTjhkss2`) only in the moments around
   launch, and is BURNED (`--final`) before any real deposit is taken — after
   which the program is immutable forever. Run the command above to verify what
   the authority is RIGHT NOW rather than trusting this file: for the live
   mainnet campaign you should expect NO authority (burned). If an authority is
   still present, the deployer can change the program; weigh that.
...
+32         dissolve_amount    u64   (USDC base units backing dissolve)
+40         proposed_payout    Pubkey (32; all-zeros = never proposed)
+72         proposal_id        u32   (epoch; bumps per propose, 0 = none)
+76         payout_vote_amount u64   (USDC base units backing the CURRENT epoch)
+84         dissolved          u8 (bool)  <- terminal; refunds open when 1
+85         released           u8 (bool)
+86         bump               u8
```

Note the two vote tallies are `u64` DOLLAR SUMS (not `u32` counts): a strict
majority is `tally * 2 > total_escrowed`.
...
and BOTH ballots (dissolve + current payout proposal), with vote weight equal
to `amount`.
...
const PROGRAM = new PublicKey("2PAg6iMEzPQnfzVmKdeUDctmmCYwts46Y5GEZBUDA4KJ");
...
const [campaign] = PublicKey.findProgramAddressSync(
  [Buffer.from("campaign"), Buffer.from("send-climbing")], PROGRAM);
...
5b. `campaign.admin` equals the organizer key
   ...
   address they intend to approve — a dollar-majority vote is what authorizes
